...
print("Imported Coding Library v" + __version__)
...
class CodingCanvas(QWidget):
    ...
    def closeEvent(self, event):
        #qt unhappy if a reference to ipainter is still hanging around on close
    ...
    #update functions that update the state of Qt based on state variables.  Call after changing relevant state variables.
...
#coding library drawing commands
#utility
...
def start_graphics(draw_func=noop, frames=1, data=None, framerate=40,
    ...
    canvas = CodingCanvas(draw_fn = draw_func, data = data, window_x=WINDOW_X, window_y=WINDOW_Y,
